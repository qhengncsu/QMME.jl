...
function kernel_logistic(y::Vector, K::Matrix, G::AbstractMatrix, λ::Real;
                  ϵ::Real=1e-9, verbose::Bool=true, tol::Real = 1e-4, adaptive=true)
  n = length(y)
  max_iters = 5000
  m = size(G,1)
  ...
  for iter = 1:max_iters
    ...
    #lin_solve_MM!(direction, grad, 0.25, λ, ϵ, e, U, s)
    ldiv!(direction, L, grad)
    @. β -= direction
    if old_obj < obj
      nesterov = 0 
...
function kernel_logistic_newton(y::Vector, K::Matrix, G::AbstractMatrix, λ::Real;
                       verbose::Bool=true, tol::Real = 1e-4, ϵ::Real = 1e-9)
  n = length(y)
  max_iters = 100
  m = size(G,1)
  (r, s) = zeros(n), zeros(n) # work vectors
