...
function FastMultinomial_reduced(X::Matrix, Y::Matrix; tol::Real=1e-6, verbose=true)
    ((n, p), q, max_iters) = (size(X), size(Y,2), 1000)
    ηs = zeros(n, q-1)
    r = zeros(n, q-1)
    P = similar(Y)
    row_sums_buffer = zeros(n)
    XTX, XTY = X'X , X'Y[:,1:(q-1)]
...
function KernelMultinomial(Y::Matrix, K::Matrix, G::AbstractMatrix,λ::Real;
                   tol::Real=1e-4, verbose=true, ϵ=1e-4, adaptive=true) 
  (n, q, max_iters) = (size(K,1), size(Y, 2), 5000)
...
function KernelMultinomial(Y::Matrix, K::Matrix, G::Matrix,λs::Vector;
                   tol::Real=1e-4, verbose=true,ϵ=1e-4, adaptive=true) 
  start_time = time()
  (n, q, max_iters) = (size(K,1), size(Y, 2), 500)
  ηs, r = zeros(n,q-1), zeros(n,q-1)
  E = 2 .*(Diagonal(ones(q-1)) + ones(q-1)*Transpose(ones(q-1)))
  (e2,V) = eigen(E)
  m = size(G,1)
...
function KernelMultinomial_newton(Y::Matrix, K::Matrix, G::Matrix, λ::Real;
                        tol::Real=1e-4, verbose=true, ϵ=1e-4) 
  (n, q, max_iters) = (size(K,1), size(Y, 2), 100)
  ηs, r = zeros(n,q-1), zeros(n,q-1)
  m = size(G,1)      
  B = zeros(m,q-1)
  direction, grad = similar(B),similar(B)
  ...
  for iter = 1:max_iters
    ...
    # Compute H2 (outer product part)
    for l in 1:(q-1)
      for k in 1:(q-1)
         # Compute the Hadamard product for V columns k and l
        p_kp_l = P[:, k] .* P[:, l]
...
function KernelMultinomial_newton(Y::Matrix, K::Matrix, G::Matrix, λs::Vector;
  ...
  for j in 1:length(λs)
    ...
    for iter = 1:max_iters
      ...
      #fill!(H, 0.0)
      #for i in 1:n
        #H .+= kron(Diagonal(P[i,1:(q-1)]) .- P[i,1:(q-1)]*Transpose(P[i,1:(q-1)]),KGT[i,:]*Transpose(KGT[i,:]))
      #end
      fill!(H1, 0.0)
      fill!(H2, 0.0)
      for k in 1:(q-1)
...
function KernelMultinomial_fista(Y::Matrix, K::Matrix, G::Matrix,λ::Real;
                        tol::Real=1e-4, verbose=true) 
  (n, q, max_iters) = (size(K,1), size(Y, 2), 3000)
  ηs, r = zeros(n,q-1), zeros(n,q-1)
  m = size(G,1)
  B = zeros(m,q-1)
  direction, grad = similar(B),similar(B)
